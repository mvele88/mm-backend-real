...
async function getJupiterQuote(inputMint, outputMint, amount, slippage = 2, onlyDirectRoutes = false) { // Changed slippage to 2%
    try {
        console.log(`[JUPITER_QUOTE] Requesting quote: InputMint=${inputMint}, OutputMint=${outputMint}, Amount=${amount}, Slippage=${slippage}, OnlyDirectRoutes=${onlyDirectRoutes}`); // Added logging
        const response = await axios.get('https://quote-api.jup.ag/v6/quote', {
            ...
            // Added a timeout of 10 seconds (10000 ms)
            timeout: 10000
        });
        ...
        if (axios.isAxiosError(error) && error.code === 'ECONNABORTED') {
            console.error(`Error fetching Jupiter quote (TIMEOUT): ${error.message}`);
        } else {
