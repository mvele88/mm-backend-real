...
// Function to get Jupiter Quote (UPDATED ENDPOINT AND ERROR LOGGING)
async function getJupiterQuote(inputMint, outputMint, amount, slippage = 2, onlyDirectRoutes = false) {
    try {
        ...
        const response = await axios.get('https://public.jupiterapi.com/v6/quote', { // MODIFIED URL HERE
            params: {
                inputMint,
                outputMint,
                amount,
        ...
        if (axios.isAxiosError(error)) {
            if (error.code === 'ECONNABORTED') {
                ...
                // IMPORTANT: Log the full response data for HTTP errors like 400
...
async function calculateRealProfit() {
    ...
    // TEMPORARILY SET TO 1 SOL FOR TESTING JUPITER QUOTES
    const inputAmountForTrade = 1; // STILL 1 SOL FOR THIS TEST
    ...
    if (quoteRes && quoteRes.data && quoteRes.data.length > 0) {
        ...
        try {
            ...
            const swapRes = await axios.post('https://quote-api.jup.ag/v6/swap', { // This is for swap, NOT quote. Keep this as is.
                route: bestRoute,
...
app.post('/start', async (req, res) => {
    ...
    if (!isRunning) {
        ...
        try {
            ...
            internalWalletKeypair = Keypair.fromSecretKey(bs58.decode(privateKeyBase58)); // CORRECTED TYPO HERE
        ...
        transactionMonitorInterval = setInterval(calculateRealProfit, 60 * 1000);
        console.log("[API /start] Sniping process interval started. Next cycle in 60 seconds.");
...
// --- NEW TEMPORARY ENDPOINT TO TEST BLOCKONOMICS API KEY ---
app.get('/test-blockonomics-api', async (req, res) => {
    ...
    try {
        const response = await axios.get('https://www.blockonomics.co/api/merchant_info', {
            ...
            timeout: 15000 // 15 seconds timeout for this test
        });
