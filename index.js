...
// ✅ UPDATED: Using your QuickNode RPC endpoint
...
// JUPITER_API_KEY is NOT needed for public Jupiter Aggregator APIs
...
const outputMintOptions = [
  ...
  'DUSTawucrTsGU8hcqRdHDCbuYhCFADMLM2VcCb8VnFnQ', // DUST
...
async function getJupiterQuote(inputMint, outputMint, amount) {
  try {
    const res = await axios.get('https://quote-api.jup.ag/v6/quote', {
      ...
      // Removed Authorization header, it's not needed for Jupiter's public API
      timeout: 20000
    });
    return res.data;
  } catch (err) {
...
// ✅ Added this route to handle GET requests to the root path
app.get('/', (req, res) => {
