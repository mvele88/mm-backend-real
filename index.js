...
// Function to fetch SOL/USD price
...
async function calculateRealProfit() {
    ...
    const inputAmountForTrade = 0.01; // Example: Try to trade with 0.01 SOL
        ...
        try {
            ...
            if (parsedTx && !parsedTx.meta.err) {
                ...
                if (outputMintTrade === 'So11111111111111111111111111111111111111112') {
                    ...
                    actualOutputSOL = (actualReceivedAmountUi * solUsdPrice) / solUsdPrice;
                }
...
app.post('/start', async (req, res) => {
    if (!isRunning) {
        ...
        try {
            ...
            console.log(`Internal wallet loaded for sniping: ${internalWalletKeypair.publicKey.toBase58()}`);
        } catch (error) {
            console.error("Failed to load internal wallet:", error.message);
            isRunning = false;
        ...
        // Changed interval to 30 seconds to reduce CoinGecko API calls
        transactionMonitorInterval = setInterval(calculateRealProfit, 30 * 1000);
